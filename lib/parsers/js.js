...
function findExported(ast) {
  ...
  const unwrap = (node, depth = 0) => {
    ...
    if (node.type === 'CallExpression' && node.arguments.length === 1 && isObjectFreeze(node.callee)) {
      return unwrap(node.arguments[0], depth + 1);
    }
    return null;
  };
...
function parseJs(source, { ignoreKeys = [] } = {}) {
  ...
  if (!root) {
    throw new ParseError(
      "Aucun `module.exports = { … }` (ni `export default { … }`) contenant un objet ou un tableau n'a été trouvé.",
    );
  }
  ...
  const add = (node, text, kind, path) => {
    segments.push({
      id: segments.length,
      text,
      path: path.join('.'),
      loc: { start: node.start, end: node.end, kind, quote: source[node.start] },
    });
  };
...
function encodeKey(key) {
  return /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(key) ? key : encodeString(key, "'");
}

/**
 * Insère de nouvelles propriétés `key: 'value'` dans l'objet exporté d'un fichier JS, juste avant
 * son accolade fermante — sans toucher au reste du fichier. Ne gère que des clés de premier niveau
 * (pas de chemin imbriqué type "a.b") : c'est la limite acceptée pour garder l'insertion fiable.
 * entries : [[clé, valeur], …]
 */
function appendJsProperties(source, entries) {
  const ast = parseProgram(source);
  const root = findExported(ast);
  if (!root) throw new ParseError("Aucun export exploitable trouvé pour y ajouter des clés.");
  if (root.type !== 'ObjectExpression') {
    throw new ParseError("Cet export est une liste (tableau) : l'ajout de clés n'est possible que sur un objet.");
  }

  const indent = /^([ \t]+)\S/m.exec(source)?.[1] ?? '  ';
  const insertAt = root.end - 1; // juste avant l'accolade fermante de l'objet exporté
  const hasExistingProps = root.properties.length > 0;
  const gap = hasExistingProps ? source.slice(root.properties.at(-1).end, insertAt) : '';
  // Une virgule traîne-t-elle déjà entre la dernière propriété et l'accolade fermante ?
  const needsLeadingComma = hasExistingProps && !gap.includes(',');
  // On insère la virgule manquante juste après la dernière valeur, avant les espaces qui suivent.
  const lastPropEnd = hasExistingProps ? root.properties.at(-1).end : insertAt;
  const before = needsLeadingComma
    ? source.slice(0, lastPropEnd) + ',' + gap
    : source.slice(0, insertAt);

  const body = entries.map(([key, value]) => `${indent}${encodeKey(key)}: ${encodeString(value, "'")},`).join('\n');
  const insertion = `${body}\n`;

  return (hasExistingProps ? before : source.slice(0, insertAt)) + insertion + source.slice(insertAt);
}

module.exports = { parseJs, appendJsProperties };